...
2. **Default FHIR package servers** — `packages.fhir.org`, `packages2.fhir.org`
3. **Custom servers** — configured in `fhir-settings.json` (see below)
...
1. Update `sushi-config.yaml`:
   ...
   ```
2. Update the version constant in `_build.sh` (`ensure_fhir_ph_core`) and in each CI workflow step.
3. Verify the new `package.tgz` is available at `https://jgsuess.github.io/ph-core/0.2.0/package.tgz`.
4. Run `./_build.sh build` locally to validate before committing.
